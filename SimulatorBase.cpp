...
void SimulatorBase::initShaders()
{
	...
	//*************//
	vertFile = getExePath() + "/resources/shaders/vs_liquid_depth.glsl";
	fragFile = getExePath() + "/resources/shaders/fs_liquid_depth.glsl";
	m_shader_liquid_depth.compileShaderFile(GL_VERTEX_SHADER, vertFile);
	m_shader_liquid_depth.compileShaderFile(GL_FRAGMENT_SHADER, fragFile);
	m_shader_liquid_depth.createAndLinkProgram();
	m_shader_liquid_depth.begin();
	m_shader_liquid_depth.addUniform("modelview_matrix");
	m_shader_liquid_depth.addUniform("projection_matrix");
	m_shader_liquid_depth.addUniform("radius");
	m_shader_liquid_depth.addUniform("viewport_width");
	m_shader_liquid_depth.end();

	vertFile = getExePath() + "/resources/shaders/vs_depthCurvatureFlowBlur.glsl";
	fragFile = getExePath() + "/resources/shaders/fs_depthCurvatureFlowBlur.glsl";
	//vertFile = getExePath() + "/resources/shaders/vs_depthBilateralCombineBlur.glsl";
	//fragFile = getExePath() + "/resources/shaders/fs_depthBilateralCombineBlur.glsl";
	m_shader_liquid_smooth_depth.compileShaderFile(GL_VERTEX_SHADER, vertFile);
	m_shader_liquid_smooth_depth.compileShaderFile(GL_FRAGMENT_SHADER, fragFile);
	m_shader_liquid_smooth_depth.createAndLinkProgram();
	m_shader_liquid_smooth_depth.begin();
	m_shader_liquid_smooth_depth.addUniform("filterRadius");
	m_shader_liquid_smooth_depth.addUniform("step");
	m_shader_liquid_smooth_depth.addUniform("projection_matrix");
	m_shader_liquid_smooth_depth.addUniform("image");
	m_shader_liquid_smooth_depth.addUniform("depthFalloff");
	m_shader_liquid_smooth_depth.end();
	
	vertFile = getExePath() + "/resources/shaders/vs_liquid_thick.glsl";
	fragFile = getExePath() + "/resources/shaders/fs_liquid_thick.glsl";
	m_shader_liquid_thick.compileShaderFile(GL_VERTEX_SHADER, vertFile);
	m_shader_liquid_thick.compileShaderFile(GL_FRAGMENT_SHADER, fragFile);
	m_shader_liquid_thick.createAndLinkProgram();
	m_shader_liquid_thick.begin();
	m_shader_liquid_thick.addUniform("modelview_matrix");
	m_shader_liquid_thick.addUniform("projection_matrix");
	m_shader_liquid_thick.addUniform("radius");
	m_shader_liquid_thick.addUniform("viewport_width");
	m_shader_liquid_thick.end();

	vertFile = getExePath() + "/resources/shaders/vs_liquid.glsl";
	fragFile = getExePath() + "/resources/shaders/fs_liquid.glsl";
	m_shader_liquid.compileShaderFile(GL_VERTEX_SHADER, vertFile);
	m_shader_liquid.compileShaderFile(GL_FRAGMENT_SHADER, fragFile);
	m_shader_liquid.createAndLinkProgram();
	m_shader_liquid.begin();
	m_shader_liquid.addUniform("texDepth");
	m_shader_liquid.addUniform("texThick");
	m_shader_liquid.addUniform("backgroundTex");
	m_shader_liquid.addUniform("modelview_matrix");
	m_shader_liquid.addUniform("projection_matrix");
	m_shader_liquid.addUniform("InvView_matrix");
	m_shader_liquid.addUniform("InvProject_matrix");
	m_shader_liquid.addUniform("liquidColor");
	m_shader_liquid.addUniform("depthFalloff");
	m_shader_liquid.end();
	
	vertFile = getExePath() + "/resources/shaders/vs_skybox.glsl";
	fragFile = getExePath() + "/resources/shaders/fs_skybox.glsl";
	m_shader_skybox.compileShaderFile(GL_VERTEX_SHADER, vertFile);
	m_shader_skybox.compileShaderFile(GL_FRAGMENT_SHADER, fragFile);
	m_shader_skybox.createAndLinkProgram();
	m_shader_skybox.begin();
	m_shader_skybox.addUniform("skybox");
	m_shader_skybox.addUniform("modelview_matrix");
	m_shader_skybox.addUniform("projection_matrix");
	m_shader_skybox.end();
	//************//
...
/**************************************/
void SimulatorBase::liquidRendering(Shader *shader)
{
	...
	shader->begin();
	glUniform1i(shader->getUniform("texDepth"), 0);
	glUniform1i(shader->getUniform("texThick"), 1);
	glUniform1i(shader->getUniform("backgroundTex"), 2);
	GLfloat matrix[16];
	glGetFloatv(GL_MODELVIEW_MATRIX, matrix);
	glUniformMatrix4fv(shader->getUniform("modelview_matrix"), 1, GL_FALSE, matrix);
	glm::mat4 InvView = glm::inverse(glm::make_mat4(matrix));
	glUniformMatrix4fv(shader->getUniform("InvView_matrix"), 1, GL_FALSE, &InvView[0][0]);
	GLfloat pmatrix[16];
	glGetFloatv(GL_PROJECTION_MATRIX, pmatrix);
	glUniformMatrix4fv(shader->getUniform("projection_matrix"), 1, GL_FALSE, pmatrix);
	glm::mat4 InvProject = glm::inverse(glm::make_mat4(pmatrix));
	glUniformMatrix4fv(shader->getUniform("InvProject_matrix"), 1, GL_FALSE, &InvProject[0][0]);
	glUniform4fv(shader->getUniform("liquidColor"), 1, &m_liquidColor[0]);
	glUniform1f(shader->getUniform("depthFalloff"), m_depthFalloff);

	glBindVertexArray(m_quad_vao);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, texDepth);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, texThick);
...
void SimulatorBase::liquidThickRendering(Shader *shader, FluidModel *model)
{
...
void SimulatorBase::liquidDepthCurvatureFlowBlur(Shader *shader)
{
	//glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
	...
	glGetFloatv(GL_PROJECTION_MATRIX, pmatrix);
	glUniformMatrix4fv(shader->getUniform("projection_matrix"), 1, GL_FALSE, pmatrix);
	glUniform1f(shader->getUniform("step"), m_curvStep);
	glUniform1i(shader->getUniform("image"), 0);
	glUniform1f(shader->getUniform("filterRadius"), 10.0f);
	glUniform1f(shader->getUniform("depthFalloff"), m_depthFalloff);
	glBindVertexArray(m_quad_vao);
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, texDepth);
...
void SimulatorBase::liquidDepthRendering(Shader *shader, FluidModel *model)
{
	glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
...
void SimulatorBase::skyRendering(Shader *shader)
{
	glBindFramebuffer(GL_FRAMEBUFFER, framebuffer);
	glEnable(GL_DEPTH_TEST);
	...
	GLfloat matrix[16];
	glGetFloatv(GL_MODELVIEW_MATRIX, matrix);
	glm::mat4 view = glm::mat4(glm::mat3(glm::make_mat4(matrix))); // remove translation from the view matrix
	glUniformMatrix4fv(shader->getUniform("modelview_matrix"), 1, GL_FALSE, &view[0][0]);
	GLfloat pmatrix[16];
	glGetFloatv(GL_PROJECTION_MATRIX, pmatrix);
	glUniformMatrix4fv(shader->getUniform("projection_matrix"), 1, GL_FALSE, pmatrix);
...
void SimulatorBase::liquidRenderingInit() 
{
	glGenTextures(1, &texDepth);
	glGenTextures(1, &texThick);
	...
	{
		getDataPath() + "/skybox/right.jpg",
		getDataPath() + "/skybox/left.jpg",
		getDataPath() + "/skybox/top.jpg",
		getDataPath() + "/skybox/bottom.jpg",
		getDataPath() + "/skybox/front.jpg",
		getDataPath() + "/skybox/back.jpg"
	};
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texThick, 0);
...
void SimulatorBase::initFluidData()
{
	...
	for (unsigned int i = 0; i < m_scene.fluidModels.size(); i++)		//读取采样后的流体模型
	{
	...
	for (auto it = fluidIDs.begin(); it != fluidIDs.end(); it++)		//读取Emitter
	{
...
void SimulatorBase::renderFluid(const unsigned int fluidModelIndex, float *fluidColor)
{
	...
	if (MiniGL::checkOpenGLVersion(3, 3))
	{
		...
		Shader *shader_liquid_depth = &m_shader_liquid_depth;
		Shader *shader_liquid_depth_blur = &m_shader_liquid_smooth_depth;
		Shader *shader_liquid_thick = &m_shader_liquid_thick;
		Shader *shader_liquid = &m_shader_liquid;
		Shader *shader_skyBox = &m_shader_skybox;
		if (m_isLiquid)
		{
			
			liquidDepthRendering(shader_liquid_depth, model);
			liquidDepthCurvatureFlowBlur(shader_liquid_depth_blur);
			liquidThickRendering(shader_liquid_thick, model);
			skyRendering(shader_skyBox);
			liquidRendering(shader_liquid);
		...
		else
		{
			...
			field = &model->getField(colorFieldName);		//颜色场：velocity、position、density、kappa et al.
