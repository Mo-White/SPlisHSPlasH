...
/****************/
void TW_CALL setLiquidRendering(const void *value, void *clientData);
void TW_CALL getLiquidRendering(void *value, void *clientData);
void TW_CALL getCurvatureStep(void *value, void *clientData);
void TW_CALL setCurvatureStep(const void *value, void *clientData);
void TW_CALL getDepthFalloff(void *value, void *clientData);
void TW_CALL setDepthFalloff(const void *value, void *clientData);
/****************/
...
void initParameters()
{
	...
	/**********************/
	std::string liquidStr = " label='Liquid Rendering' key=l help='Toggle LiquidRendering mode.' group='" + model->getId() + "'";
	TwAddVarCB(MiniGL::getTweakBar(), "LiquidRendering", TW_TYPE_BOOL32, setLiquidRendering, getLiquidRendering, NULL,liquidStr.c_str());

	std::string CurvStepStr = " label='Curvature Step'  step=0.00001 precision=5 help='Toggle Curvature Step Value.' group='" + model->getId() + "'";
	TwAddVarCB(MiniGL::getTweakBar(), "CurvatureStep", TW_TYPE_FLOAT, setCurvatureStep, getCurvatureStep, NULL, CurvStepStr.c_str());

	std::string DepthFalloffStr = " label='Depth Fall off'  step=0.0001 precision=4 help='Toggle DepthFalloff Value.' group='" + model->getId() + "'";
	TwAddVarCB(MiniGL::getTweakBar(), "DepthFalloff", TW_TYPE_FLOAT, setDepthFalloff, getDepthFalloff, NULL, DepthFalloffStr.c_str());
	/*********************/
...
void timeStep ()
{
	...
	for (unsigned int i = 0; i < numSteps; i++)
	{
		...
		SPH::Simulation::getCurrent()->getTimeStep()->step();		//模拟时间步
		STOP_TIMING_AVG;
...
void render()
{
	...
	if (base->m_isLiquid)
	{
		glBindFramebuffer(GL_FRAMEBUFFER, base->getFBO());
	}
	else
	{
...
void TW_CALL getLiquidRendering(void *value, void *clientData)
{
	*(int *)(value) = base->m_isLiquid == true;
}

void TW_CALL setCurvatureStep(const void *value, void *clientData)
{
	const float val = *(const float *)(value);
	base->setCurvatureStep(val);
}

void TW_CALL getCurvatureStep(void *value, void *clientData)
{
	*(float *)(value) = base->getCurvatureStep();
}

void TW_CALL setDepthFalloff(const void *value, void *clientData)
{
	const float val = *(const float *)(value);
	base->setDepthFalloff(val);
}

void TW_CALL getDepthFalloff(void *value, void *clientData)
{
	*(float *)(value) = base->getDepthFalloff();
}
